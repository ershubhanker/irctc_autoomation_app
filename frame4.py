...
def start4(user4,user4_pass,passanger_value,upi_value,method_pay,debit_value):
    ...
    try:    
        ...
        os.system(f'start cmd /k "brave.exe --remote-debugging-port=9224 --user-data-dir={var}\chrome\chromedata4"')
    ...
    def  logincaptcha():
        ...
        img.screenshot(f"{var}\logo4.png")
        im = Image.open(f"{var}\logo4.png") #use PIL.Image.open if not work
        left = 0
        top = 250
        right = 300
        bottom = 270
        ...
        im1.save(f"{var}\crop4.png" ,quality=100)
        print('image address:',im1) 
        captcha = pytesseract.image_to_string(im1) 
        captcha = captcha.replace(" ", "").strip()
        # save in abc4.txt file
        with open(f"{var}\\abc4.txt",mode ='w') as file:     
            file.write(captcha) 
...
>>>>>>> 1f7b9bd93eb137a10a592b9a9ba6e517c4a5b3c8
    ...
    time.sleep(3)
    try:
        if driver.find_element(By.XPATH, '//*[@id="ui-panel-12-content"]/div/div[1]/div[2]/div/app-passenger/div/div[1]/span/div[1]/p-autocomplete/span/input').is_displayed()==False:
            time.sleep(3)
        else:
            pass
    except:
    ...
    def upi_function():
        ...
        def paycaptcha():
            ...
            #save in payment4.txt
            with open(f"{var}\payment4.txt",mode ='w') as file:      
                file.write(captcha) 
                print('result',captcha)
