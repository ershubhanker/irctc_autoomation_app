...
def start3(user3,user3_pass,passanger_value,upi_value,method_pay,debit_value):
    ...
    try:    
        ...
        os.system(f'start cmd /k "brave.exe --remote-debugging-port=9223 --user-data-dir={var}\chrome\chromedata3"')
    ...
    def  logincaptcha():
        ...
        img.screenshot(f"{var}\logo3.png")
        im = Image.open(f"{var}\logo3.png") #use PIL.Image.open if not work
        left = 0
        top = 250
        right = 300
        bottom = 270
        ...
        im1.save(f"{var}\crop3.png" ,quality=100)
        print('image address:',im1) 
        captcha = pytesseract.image_to_string(im1) 
        captcha = captcha.replace(" ", "").strip()
        # save in abc3.txt file
        with open(f"{var}\\abc3.txt",mode ='w') as file:     
            file.write(captcha) 
...
>>>>>>> 41690c6f99fe67aaf4f1663d1e6c82eb59dfbd29
    ...
    def upi_function():
        ...
        def paycaptcha():
            ...
            #save in payment3.txt
            with open(f"{var}\payment3.txt",mode ='w') as file:      
                file.write(captcha) 
                print('result',captcha)
